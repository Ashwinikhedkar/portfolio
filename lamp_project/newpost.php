...
//populates subcategory option drop down box on newpost form
...
//populates category drop down box on newpost form
...
//populates country box on newpost form
...
//populates city drop down box on newpost form
function addCityOptions() {
		$cities = array("Cupertino",
                   "Sacramento" ,
		               "New York",
				           "Delhi" ,
		                "Mumbai",
...
function validate_cats($main_cat, $sub_cat) {
 ...
 function validate_locations($country, $city) {
      // Check if a ROW with $country and $city exist in 'locations' SQL Table.
      $link=mysql_connect('localhost','lamp','1');
      if (!$link) {
        die('Could not connect: ' . mysql_error());
      }
  ...
  // returns true: when there are no errors
    ...
    function validate_all() {
	    global $errors;
	    global $title, $description,$category, $subcategory, $country, $city, $price;
      ...
      if(!validate_locations($country,$city))
      {
	    ...
	    //check value entered in price is numeric 
	    if (!is_numeric($price))
	    { 
		    $errors['price']="Enter Numeric value";
		    $founderror=true;
	    } 
	    //validate the email
...
<!-- if not validated create a newpost form and echo the error if any. if validated post the data to preview page  -->
	...
	?>
      ...
      <tr>
        ...
        <?php
                if (!$validated) {
        ?>
          	<input type="text" name="price" value="<?php echo getPostedValue('price') ?>">
	        	<div class="inputerror"><?php echo getError('price');?></div>
      	<?php
	      	} else {
                   echo getPostedValue('price');
        	?>
	        <input type="hidden" name="price" value="<?php echo getPostedValue('price'); ?>">
        	<?php
      	        	}
        	?>
        </td>
      ...
      <tr>
       ...
       <input type="hidden" name="pic1" value="<?php echo getFileName('pic1');?>">
      <?php
        }
        ?>
        </td>
